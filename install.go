...
outerLoop:
	...
	if len(rebootList) > 0 {
		...
		if osType == glazos.Client {
			// If the active hours end time is in the future, use the end time. Otherwise, use the same end time of the next day.
			if ah[0].Closes.After(time.Now()) {
				rebootTime = ah[0].Closes
			} else {
				rebootTime = ah[0].Closes.Add(time.Hour * time.Duration(24))
			}
		} else {
