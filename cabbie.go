...
var (
	...
	eventlog         = flag.Bool("eventlog", true, "Write logs to Event log")
	config           = new(Settings)
...
func (s *Settings) regLoad(path string) error {
	...
	if a, _, err := k.GetStringValue("AukeraName"); err == nil {
		...
		logger.Info(fmt.Sprintf("AukeraName not found in registry, using default Name:\n%v", s.AukeraName))
	}

	if m, _, err := k.GetStringsValue("RequiredCategories"); err == nil {
		s.RequiredCategories = m
	} else {
		logger.Info(fmt.Sprintf("RequiredCategories not found in registry, using default categories:\n%v", s.RequiredCategories))
	}
...
func startService(isDebug bool) error {
	logger.Info(fmt.Sprintf("Starting %s service.", cablib.SvcName))
	run := svc.Run
	if isDebug {
		run = debug.Run
	}
	...
	logger.Info(fmt.Sprintf("%s service stopped.", cablib.SvcName))
	return nil
}
...
func initMetrics() error {
	...
	if err != nil {
		logger.Error(fmt.Sprintf("unable to create enforcementWatcherFailures metric: %v", err))
	}
...
func runMainLoop() error {
	if err := notification.CleanNotifications(cablib.SvcName); err != nil {
		logger.Error(fmt.Sprintf("Error clearing old notifications:\n%v", err))
	}

	if config.EnableThirdParty == 1 {
		if err := enableThirdPartyUpdates(); err != nil {
			logger.Error(fmt.Sprintf("Error configuring third party updates:\n%v", err))
		}
	}
	...
	go func() {
		for {
			...
			logger.Error(fmt.Sprintf("failed to initialize enforcement config watcher; relying on enforcement schedule: %v", err))
			if err := enforcementWatcherFailures.Increment(); err != nil {
				logger.Error(fmt.Sprintf("unable to increment enforcementWatcherFailures metric: %v", err))
			}
	...
	if config.AukeraEnabled == 1 {
		logger.Info("Host configured to use Aukera. Ignoring default timer.")
		t.Default.Stop()
	} else {
		logger.Info("Using default update interval.")
		t.Aukera.Stop()
	}
	...
	for {
		...
		case <-t.Default.C:
			...
			setRebootMetric()
			if err != nil {
				logger.Error(fmt.Sprintf("Error installing system updates:\n%v", err))
			}
		case <-t.Aukera.C:
			s, err := client.Label(int(config.AukeraPort), config.AukeraName)
			if err != nil {
				logger.Error(fmt.Sprintf("Error getting maintenance window %q with error:\n%v", config.AukeraName, err))
				break
			}
			if *runInDebug {
			...
			if len(s) == 0 {
				logger.Error(fmt.Sprintf("Aukera maintenance window label %q not found, skipping update check...", config.AukeraName))
				break
			}
			if s[0].State == "open" {
				i := installCmd{Interactive: false}
				err := i.installUpdates()
				if e := updateInstallSuccess.Set(err == nil); e != nil {
					logger.Error(fmt.Sprintf("Error posting updateInstallSuccess metric:\n%v", e))
				}
				setRebootMetric()
				if err != nil {
					logger.Error(fmt.Sprintf("Error installing system updates:\n%v", err))
		...
		case <-t.List.C:
			...
			if err := requiredUpdateCount.Set(int64(len(requiredUpdates))); err != nil {
				logger.Error(fmt.Sprintf("Error posting requiredUpdateCount metric:\n%v", err))
			}

			if len(requiredUpdates) == 0 {
				logger.Info("No required updates needed to install.")
				break
			}

			logger.Info(fmt.Sprintf("Found %d required updates.\nRequired updates:\n%s\nOptional updates:\n%s",
				len(requiredUpdates),
		...
		case <-t.Virus.C:
			...
			if e := virusUpdateSuccess.Set(err == nil); e != nil {
				logger.Error(fmt.Sprintf("Error posting virusUpdateSuccess metric:\n%v", err))
			}
			if err != nil {
				logger.Error(fmt.Sprintf("Error installing virus definitions:\n%v", err))
				break
			}
		case <-t.Driver.C:
			...
			if e := driverUpdateSuccess.Set(err == nil); e != nil {
				logger.Error(fmt.Sprintf("Error posting driverUpdateSuccess metric:\n%v", e))
			}
			if err != nil {
				logger.Error(fmt.Sprintf("Error installing drivers:\n%v", err))
			}
			setRebootMetric()
		case file := <-enforcedFile:
			kbs, err := allEnforcements()
			if err != nil {
				logger.Error(fmt.Sprintf("Error retrieving required updates from %q:\n%v", file, err))
			}
			if err := kbs.install(); err != nil {
				logger.Error(fmt.Sprintf("Error enforcing required updates:\n%v", err))
			}
		case <-t.Enforcement.C:
			kbs, err := allEnforcements()
			if err != nil {
				logger.Error(fmt.Sprintf("Error gathering required updates:\n%v", err))
			}
			if err := kbs.install(); err != nil {
				logger.Error(fmt.Sprintf("Error enforcing required updates:\n%v", err))
			}
		case <-rebootEvent:
			go func() {
				if !(rebootActive) {
					...
					if err != nil {
						logger.Error(fmt.Sprintf("Error getting reboot time: %v", err))
					}
					if t.IsZero() {
						logger.Info("Zero time returned, no reboot defined.")
						return
					}
					if err := cablib.SystemReboot(t); err != nil {
						logger.Error(fmt.Sprintf("SystemReboot() error:\n%v", err))
					}
					rebootActive = false
				}
			}()
...
loop:
	for {
		...
		case err := <-errch:
			logger.Error(fmt.Sprintf("Cabbie goroutine has failed: %v", err))
			break loop
...
func main() {
	...
	logger := logger.Init(cablib.LogSrcName, *verbose, true, ioutil.Discard)
	defer logger.Close()

	// Load Cabbie config settings.
	config = newSettings()
	if err = config.regLoad(cablib.RegPath); err != nil {
		logger.Warning(fmt.Sprintf("Failed to load Cabbie config, using defaults:\n%v\nWarning: %v", config, err))
	}
	...
	if err != nil {
		logger.Error(fmt.Sprintf("Failed to determine if we are running in an interactive session: %v", err))
		os.Exit(2)
	}
	...
	if isSvc && len(os.Args) == 1 {
		if err := startService(*runInDebug); err != nil {
			logger.Error(fmt.Sprintf("Failed to run service: %v", err))
			os.Exit(2)
		}
		os.Exit(0)
	}
	...
	if *runInDebug {
		if err := startService(true); err != nil {
			logger.Error(fmt.Sprintf("Failed to run service in debug mode: %v", err))
			os.Exit(2)
		}
	}
