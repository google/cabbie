...
func createTestKeys(t *testing.T) error {
	t.Helper()
...
func cleanupTestKey(t *testing.T) error {
	t.Helper()
...
func TestAddGetRebootUpdatesSuccess(t *testing.T) {
	// Setup
	RegPath = testPath
	if err := createTestKeys(t); err != nil {
		t.Fatal(err)
	}
	...
	if !reflect.DeepEqual(got, fakeUpdates) {
...
func TestAddGetRebootUpdatesFailure(t *testing.T) {
	...
	_, _, err = k.GetStringsValue(`RebootUpdates`)
